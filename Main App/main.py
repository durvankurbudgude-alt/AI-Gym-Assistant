...
def autoplay_audio(audio_source):
    """
    Converts raw server audio bytes or files generated by your TextToSpeech engine
    into an immediate, unblockable native HTML5 autoplay playback.
    """
    if not audio_source:
        return
    
    try:
        # Check if it's a file path or raw bytes stream
        if isinstance(audio_source, str) and os.path.exists(audio_source):
            ...
            # Fallback for file-like stream objects
            try:
...
def main():
    ...
    if st.session_state.get("audio_to_play"):
        # Play raw sound bytes generated by your TTS engine instantly
        autoplay_audio(st.session_state.audio_to_play)
        # Wipe it immediately so it doesn't loop during the camera loop reruns
        st.session_state["audio_to_play"] = None
