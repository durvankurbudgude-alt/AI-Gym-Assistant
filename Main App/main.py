...
def main():
    ...
    if "voice_pipeline" not in st.session_state or st.session_state.voice_pipeline is None:
        try:
            ...
            st.sidebar.success("✅ Voice Pipeline Connected Successfully!")
        except Exception as err:
            st.session_state.voice_pipeline = None
            st.sidebar.error(f"❌ PIPELINE CRASHED: {str(err)}")
    else:
        st.sidebar.success("✅ Voice Pipeline Active and Engaged.")
